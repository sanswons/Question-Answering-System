...
sub read_anskey {
    my($file) = @_;
    my($qid, $question, $answer, $difficulty, $type, $keyinfo);
    my(@keyinfo_list) = ();
	...
	if ($line =~ /^Type:\s*(.*?)\s*$/i) {
	    ...
	    print "ERROR: Expected Type but found: $line\n";
	}
	$keyinfo = create_keyinfo($qid, $question, $answer, $difficulty, $type);
...
sub create_keyinfo {
    my($qid, $question, $answer, $difficulty, $type) = @_;
    my($keyinfo);
...
sub print_keyinfo {
    my($keyinfo) = @_;
    my($qid, $question, $answer, $difficulty, $type);
